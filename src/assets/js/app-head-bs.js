...
const isDarkMode = () => {
    return document.documentElement.classList.contains('uni-dark');
};
const setDarkMode = (enableDark = true) => {
    const isDark = isDarkMode();
    if (isDark === enableDark) return;
    if (enableDark) {
        document.documentElement.classList.add('uni-dark');
    } else {
        document.documentElement.classList.remove('uni-dark');
    }
...
if (!ENABLE_PAGE_REVEALER_USED && ENABLE_PAGE_PRELOADER) {
    ...
    const duration = 1.1; // seconds
    {
        ...
        .uni-preloader.uni-dark,
        .uni-dark .uni-preloader {
